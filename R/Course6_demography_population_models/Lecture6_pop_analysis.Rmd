...
### describe the variation of population size, density & demographic parameters in time
---
# Content

--

### From abundance to population size
--

### Detactability issues 
--
...
#### this specific model (constant $p$) is often used as a NULL model ( $M_0$ )
...
+ This are already 7 parameters to estimate  ( $\hat{N}$ + $p_{1}$ + $p_{2}$ + $p_{3}$ + $p_{4}$ + $p_{5}$ + $p_{6}$ )
--
...
+ E.g. estimating survival ( $\phi_{i}$ ), i.e. an animal was marked at sampling occassion $i$, survives until sampling occasion $i + 1$ & is still part of the population 
--

+ our detection probability $p$ remains dependent on single sampling occasions
---
class: center
...
+ Often, we wish to explain heterogeneity in survival based on sex, age-class, social status, etc. (i.e discrete covariates)
--


+ e.g. estimate survival based on social status for each individual (low = 0, high = 1):

$$logit(\phi_{i}) = \beta_{1} + \beta_{2} rank_{i}$$ 

+ Here, we use the logit link function to estimate parameters between the probability scale (0,1)
...
+ State transition is described in transition matrices :
...
\hspace{-1.2em}
\begin{matrix}
    z_t=BP_{neg} & z_t=BP_{pos} & z_t=D \\ \hdashline
\phi_{BP_{neg}} (1-\psi_{BP_{negpos}}) & \phi_{BP_{neg}} \psi_{BP_{negpos}} & 1 - \phi_{BP_{neg}}\\ 
\phi_{BP_{pos}} \psi_{BP_{posneg}} & \phi_{BP_{pos}} (1-\psi_{BP_{posneg}}) & 1 - \phi_{BP_{pos}}\\ 
0 & 0 & 1
\end{matrix}
\hspace{-0.2em}
\begin{matrix}
...
\left . \vphantom{ \begin{matrix} 12 \\ 12 \\ 12 \end{matrix} } \right )
    \begin{matrix}
    z_{t-1}=BP_{neg} \\ z_{t-1}=BP_{pos} \\ z_{t-1}=D
    \end{matrix}
\end{matrix}
$$
--

+ Observation process & disease uncertainty is described in observation matrices :
...
1-p_{BP_{neg}} & p_{BP_{neg}} \beta_{Test} & 0 & p_{BP_{neg}} (1-\beta_{Test})\\ 
1-p_{BP_{pos}} & 0 & p_{BP_{pos}} \beta_{Test} & p_{BP_{pos}} (1-\beta_{Test})\\ 
1 & 0 & 0 & 0
\end{matrix}
\hspace{-0.2em}
\begin{matrix}
...
\left . \vphantom{ \begin{matrix} 12 \\ 12 \\ 12\end{matrix} } \right )
    \begin{matrix}
    z_{t}=BP_{neg} \\ z_{t}=BP_{pos} \\ z_{t}=D
    \end{matrix}
\end{matrix}
$$
---
