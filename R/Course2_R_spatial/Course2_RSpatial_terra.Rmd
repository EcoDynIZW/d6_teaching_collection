...
packages — namely `{terra}` and `{stars}`. The `{terra}` package is also especially useful for remote sensing data; it has a single class `SpatRaster` class. The course is updated to use the `{terra}` package. 
...
## Geeky stuff for coders

Important is the first line here for setting the namespace. Sometimes, packages use the same
...
The other lines show what objects the function can use.
...
# filenames
```
Should this be empty, then you have wrongly set the root directory and the data directory. Please check again how your folder structure looks like. By default, this folder should contain six ascii-files. The description what info these maps contain is provided in the DataDescription_Borneo.doc in the 
...
Now have a look at the slot `crs` (CRS stands for coordinate reference system): Nicely in the new `{terra}`-package, the Coordinate Reference System (map projection) is automatically set in the following way: If this argument is missing when loading data with `rast(x= ..., crs= ...)`, and the x coordinates are within -360 .. 360 and the y coordinates are within -90 .. 90, automatically "+proj=longlat +datum=WGS84" is used.

In this case, (OGC:CRS84) stands for 'Open Geospatial Consortium' standards to represent the *c*oordinate *r*eference *s*ystem (CRS) WGS84. The parameters for this CRS are now stored in so-called 'well-known text' (WKT) files. We will come back to that later. 

However, sometimes the slot 'coord. ref' is empty, i.e. if other coordinates than decimal degrees are used, e.g. UTM. That means, without knowing the reference system, the map cannot be
...
e.g. `st_buffer()`, `st_intersection()` etc.
...
```{r relief-rnaturalearth, fig.height=4.5, eval=FALSE}
