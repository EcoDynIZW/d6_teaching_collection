...
# dbor_wd     <- paste0(root_wd, "/", "data/data_borneo") ## the old way to define the path from the root
...
In this case, it was automatically assigned, so nothing to do for now.But see later, when we do not have a geographic CRS underlying the map....
...
crs(ras_bio_asc_01_cr) ## != CRS(ras_bio_asc_01_cr) ## mind the spelling in capital letters
...
We will now work with the digital elevation model (DEM) `ras_bio_asc_24`, where each raster cell is the height above sea level. <br>.

Slope = steepness or degree of incline of a surface,influences water flow and erosion.
Aspect = compass direction that a slope faces, measured in degrees clockwise from north (e.g., 0° is north, 90° is east, 180° is south, 270° is west). E.g. important for sunlight exposure. 

Both are fundamental topographical characteristics used together to describe terrain and are widely applied in fields like geography, land development, and environmental science. 

Further reading: https://gisnavigator.co.uk/slope-and-aspect-terrain-analysis/ or 
https://zia207.quarto.pub/digital-terrain-analysis.html 

for visualisation, we will create a hillshade (3D look) based on topography using slope and aspect:
...
Based on elevational maps, also a viewshed-analysis can be performed. <br>
https://rdrr.io/cran/terra/man/viewshed.html <br>


```{r}

bor_dem_proj      <- project(ras_bio_asc_24, "+proj=moll +lat_0=65 +lon_0=10") 
## see chapter below for explanation
plot(bor_dem_proj) ## check dimension of x and y coordinates and choose center

observer_location <- cbind(10500000 , 0) ## 0 is equator

observer_height   <- 500 ## height above the elevation data of the observer 
target_height     <- 1000 ## height above the elevation data of the targets


vs_raster <- terra::viewshed(x = bor_dem_proj, 
...
plot(vs_raster)
points(observer_location[2] ~ observer_location[1], col='red', cex = 2)
```
...
tmap_mode(mode = "view")
tm_shape(shp = ras_bio_asc_01) + tm_raster(col_alpha=0.5)
```
...
## store it as object i.m and then save it
...
g <- ggplot() +
  ...
  coord_sf(crs = "EPSG:4326") + ## set correct projection 
...
## `ggsave()` automatically saves the last plot if not specified otherwise. 
## (You can also specify a ggplot object via `plot = `).
...
There is a new package called `tidyterra`, where *SpatRaster*-objects of `terra` can be plotted directly without the detour via `stars`. for an example see here: https://dieghernan.github.io/tidyterra/


Further beautiful ideas with ggplot for contour plots:
...
ras_bio_asc_01_agg <- aggregate(x = ras_bio_asc_01, fact = 50, fun = mean) 
...
```{r binary-plot}
plot(x = mean.t.c.25)
range(values(mean.t.c.25), na.rm =TRUE)
```
...
Remember the function **`extract()`** from the lecture. In the following, we extract from the
small (cropped) raster the first three values (= cols 1-3; longitude or x-axis) 
...
xy <- terra::xyFromCell(object = ras_bio_asc_01_cr, cell = cells) ## same as: 
#xy <- terra::crds(ras_bio_asc_01_cr)[cells,]
xy
class(xy)
...
## to get an overview over all possible functions to get access to the raster
...
## combine in one table that can be used for further statistical analysis
...
points(sv_xy, col= 'white', pch= 19, cex = 2)
```
...
# viz
plot(my_dist) #units?
points(sv_xy_2, col= 'white', pch= 19, cex = 2) 
```

## Categorical rasters

Let's go back to our raster with the temperature below or above 25° C. It is a binary map with categories (above or below 25°C), although hard-coded via the numbers 0 and 1. This can be transformed into a factors:

```{r}
mean_f_ras <- as.factor(mean.t.c.25)
levels(mean_f_ras)

## create a level look-up table
cls <- data.frame(ID=0:1, mean_t = c("below","above"))
levels(mean_f_ras) <- cls
is.factor(mean_f_ras)

plot(x=mean_f_ras)
```

In the same way, a legend e.g. for several land use categories could be defined.

### Quantifying landscape heterogeneity 

See separate course. Possible with package `landscapemetrics`: https://r-spatialecology.github.io/landscapemetrics/
based on FRAGSTATS: https://www.fragstats.org/

For example, calculate the Shannon diversity index:
```{r}
#install.packages('landscapemetrics')
#library(landscapemetrics)
#check_landscape(mean_f_ras) ## by now, you understand what the error message means....
#mean_f_ras_proj <- terra::project(mean_f_ras, 'EPSG:3085') 

#check_landscape(mean_f_ras_proj) ## check again
#lsm_l_shdi(mean_f_ras_proj) ## 0.6 

## simply do it by hand
#terra::summary(mean_f_ras_proj) ## take out the values
#n <- c(7017,21712)
#p <- n / sum(n)
...
## save the small cropped and aggregated rasters
...
Borneo_shp <- sf::st_read(dsn = vecs_wd, layer = "borneo_admin")[, c(1:3, 5, 7, 17, 18)] 
...
In the following, we load two point data-sets of different species locations.

```{r check-projection}
pt_shp <- st_read(dsn = paste0(anim_wd, "/FCsim.shp")) ## CRS is missing!
head(pt_shp)  ## attribute table with geometry column
...
either plot just one column with the geometry or define it as `{sp}` object:

```{r base-plot-sf-1, fig.height=7}
# plot(Borneo_shp) ## not run!
plot(Borneo_shp[,1]) ## select column 1 or 
plot(st_geometry(Borneo_shp))
```
...
The site ID is colour-coded here in a gradient. For further reading:
...
tmap_mode(mode = "view")
tm_shape(shp = PA_shp[, 1]) + 
  tm_polygons(fill = "SITE_ID", fill.scale  = tm_scale_continuous_log10())
```
...
tm_shape(shp = Borneo_shp_moll) + 
  tm_polygons(col = "blue") ## do you see the difference?
```
...
nrow(recs_sf) ## 500
insidePA <- st_intersection(x = recs_sf, y = PA_shp) ## takes some seconds ##what does the warning mean?
nrow(insidePA) ## 11
```
...
tm_shape(shp = ras_bio_asc_24, raster.downsample = TRUE) +
    tm_raster(col.scale  = tm_scale_continuous(values = "viridis"), col_alpha = 0.1) +
tm_shape(shp = ras_bio_asc_01, raster.downsample = TRUE) + 
    tm_raster(col.scale = tm_scale_continuous(values ="turbo"), 
              col_alpha = 0.3) +
tm_shape(shp = ras_bio_asc_01_cr_sf) + 
  tm_symbols(shape = 1, size = 0.05, col= 'white') ## or tm_dots()
```
...
https://tmap.geocompx.org/layers
...
To do: transform tmap v3 code into v4:
...
```{r, eval=FALSE}
## Set/ assign the CRS if missing -> here with EPSG-code
crs(myraster)  <- "4326" ##assign if raster comes without CRS
st_crs(myshapefile, '3085')
