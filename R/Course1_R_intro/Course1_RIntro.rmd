...
Please note: for now, this is just text (a character string denoted with ' ') pointing to a location in your directory, not yet a folder! Create the folder either in your explorer manually, or more elegantly with `dir.create()`. To avoid overwriting existing folders, ask first if the folder exists. 
...
Very helpful if you want to add data or change the value of existing data. We are assigning our numbers now to an object which will contain the wingspans of dragonflies.


![dragonfly](http://phylopic.org/assets/images/submissions/00415ca0-dbca-4312-b871-840fe1c7031b.512.png)
...
What happens: the mathematical operation is applied to each pointer (= measurement) in your vector (note the difference to a function like `sum()`, which was summing the values of the vector).
...
Do the following so that we create a big data file, which will be called 'insect_type', and which will contain the wing spans of butterflies and dragonflies.
...
Very helpful commands if you want to know what is stored in your object.
```{r, tidy=TRUE}
