...
```{r}
## package names:
pkgs = c("here", "lubridate", "sf", "sp", "raster", "ggplot2", "tmap", "circular", "move", "adehabitatHR", "viridis", "devtools", "plotly","move2", "amt", "terra","effects","tidyterra") 
...
#library(tidyterra)     ## for visualizing raster objects in ggplot
...
## speed
max(speed(my_fox),na.rm=TRUE) ## 0.8, measured in m/s
...
b_env_var_stack <- rast(list(b_imperv,b_sutemp, b_forest,b_noise))

## make quick plot of stack
plot(b_env_var_stack) 

## single plot with points
plot(b_env_var_stack[[4]])
...
## change names:
colnames(df_rsf_fox_24) <- c("case_" , "x_" , "y_", "imp", "stmp", "tree", "noise")
df_rsf_fox_24$case_num <- ifelse(df_rsf_fox_24$case_ == 'FALSE',0,1)
```

Run the RSF. scale variables before so that they all have the same range of values
...
Plot the effects and odds ratio. An odds ratio of 1 indicates that the condition or event 
under study is equally likely to occur in both groups. 
An odds ratio greater than 1 indicates that the condition or event 
is more likely to occur in the first group.
```{r}

library(effects)
plot(allEffects(rsf_fox),lty=1)


#sjPlot::plot_model(rsf_fox, transform = "plogis", type="std")
sjPlot::plot_model(rsf_fox,  
                   vline.color = "blue",line.size = 1.5,dot.size = 3,
...
mydf1 <- ggeffects::ggpredict(rsf_fox) #, terms = c("imp", "stmp","tree", "noise"))
plot(mydf1$imp) ## analoguously for the rest
 
 
## forest plot
ggplot(rsf_fox, aes(x=reorder(term, estimate), y=estimate)) +
