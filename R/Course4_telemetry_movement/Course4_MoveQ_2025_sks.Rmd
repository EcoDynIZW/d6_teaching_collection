...
```{r}
## package names:
pkgs = c("here", "lubridate", "sf", "sp", "raster", "ggplot2", "tmap", "circular", "move", "adehabitatHR", "viridis", "devtools", "plotly","move2", "amt", "terra") 
...
```{r}
maps_wd   <- here("data","data_berlin","geo_raster_current_asc")  
lf <- list.files(maps_wd, pattern = '.asc', full.names = TRUE); lf
b_sutemp <- terra::rast(lf[1]) 
b_forest <- terra::rast(lf[2])
b_sutemp ## coord. ref. is empty
crs(b_sutemp) <-  "epsg:3035"
crs(b_forest) <-  "epsg:3035"
b_sutemp ## crosscheck
