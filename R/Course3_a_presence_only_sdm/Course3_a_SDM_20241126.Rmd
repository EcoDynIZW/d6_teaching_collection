...
predictors_tmp <- terra::rast(x = c(files[c(1:6)]) ) 

## convert to raster stack, as MaxEnt does not work with terra yet
predictors_selected <- raster::stack(predictors_tmp) 
```
...
```{r}
pred_no_bias_current <- terra::mean(raster::predict(xm_nobias_current, predictors_selected))
...
res_plot <- sneak - terra::rast(x = pred_no_bias_current)
...
```
 ...
 TODO - stopped here - need to run prior code
 
# After results generated - CONTINUE HERE

TODO - write that we can cnotinue irrespective of R or manual running Maxent
