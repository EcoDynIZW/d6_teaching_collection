...
For all of them is interpolated or extrapolated as requested and we get the values in qD with lower and upper confidence intervales. 
...
TODO - if this is coverage, shouldn't the object also be called estiNEXTcov, or so? see below
...
```{r}
## TODO  - something has changed here in the package!!!!!!!! I gave up here
...
estINEXTcover2 <- estimateD(spider, datatype = "abundance", base = "size", level = 252,
          conf = 0.0)# Remove the confidence intervals w dirty trick


habitat <- factor(c("Girdled", "Logged"))

mysub <- subset(estINEXTcover2,estINEXTcover2$Order.q == 0 )
plot(mysub$qD ~ habitat, col = c("green4", "red"), 
...
birds_est_sprich <- as.data.frame(cbind(site = colnames(bird_data),
                                        sp_rich = birds_estINEXTsize$`q = 0`))
...
tree_raster   <- rast("data","data_berlin","geo_raster_current_gtif","tree_cover_density_2012_100m_3035.tif")
imperv_raster <- rast("data","data_berlin","geo_raster_current_gtif","imperviousness_2012_100m_3035.tif")
noise_raster  <- rast("data","data_berlin","geo_raster_current_gtif","noise_daynight_2017_100m_3035.tif")
water_raster  <- rast("data","data_berlin","geo_raster_current_gtif","water_bodies_2010_100m_3035.tif")

#put all environmental rasters together for the prediction
my_env_stack<- stack(tree_raster, imperv_raster, noise_raster)
...
# plot map
plot(sp_rich_pred, breaks = seq(5, 55, by = 10), col = my_palette)
